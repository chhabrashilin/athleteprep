...
import { rebuildCricketPlayerStats } from "@/lib/cricket/stats/actions";
...
export function RebuildPlayerStatsButton({ leagueId, variant = "default", onComplete }: Props) {
  ...
  async function handleRebuild() {
    ...
    setMessage(null);
    try {
      const result = await rebuildCricketPlayerStats({
        leagueId,
      ...
      if (result.success && result.data) {
        setMessage(`Rebuilt: ${result.data.playersUpdated} players, ${result.data.matchesProcessed} matches.`);
        onComplete?.();
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button onClick={handleRebuild} disabled={loading} className={baseClass}>
        ...
        {loading ? "Rebuilding…" : "Rebuild Player Stats"}
      </button>
      {message && (
