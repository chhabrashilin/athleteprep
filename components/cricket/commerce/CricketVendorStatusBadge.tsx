const CONFIG: Record<string, { label: string; color: string }> = {
  pending:   { label: "Pending Review", color: "text-amber-400  bg-amber-400/10 border-amber-500/30"  },
  ...
  suspended: { label: "Suspended",      color: "text-orange-400 bg-orange-400/10 border-orange-500/30"},
  archived:  { label: "Archived",       color: "text-slate-500  bg-slate-800    border-slate-700"     },
};

interface Props { status: string; className?: string }

export function CricketVendorStatusBadge({ status, className = "" }: Props) {
