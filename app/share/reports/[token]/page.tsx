...
function SharedReportView({ model }: { model: SharedReportViewModel }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Footer */}
      <div className="border-t border-slate-800/60 pt-6 flex flex-col items-center gap-2 text-center">
        <p className="text-xs text-slate-600">
          Generated by <span className="text-slate-500 font-medium">GameIQ</span> — AI game review for serious teams.
        </p>
        <a
          href="/feedback"
          className="text-xs text-slate-700 hover:text-slate-400 transition-colors"
        >
          Have feedback on this report or GameIQ? Share it →
        </a>
      </div>
    </div>
  );
